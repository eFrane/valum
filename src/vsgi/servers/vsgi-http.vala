...
namespace VSGI.HTTP {
	...
	private class MessageBodyOutputStream : OutputStream {
		...
		public MessageBodyOutputStream (Message message, Soup.Server server) {
			Object (message: message, server: server);
		}
	...
	private class Request : VSGI.Request {
		...
		public ClientContext client { construct; get; }
		 ...
		 */
		public Request (Message msg, HashTable<string, string>? query, ClientContext client) {
			Object (message:           msg,
			        client:            client,
			        ...
			        body:              new MemoryInputStream.from_data (msg.request_body.flatten ().data));
		}

#if SOUP_2_50
		public override IOStream steal_connection () {
			return client.steal_connection ();
		}
#endif
	}
	...
	private class Response : VSGI.Response {
		 ...
		 */
		public Response (Request req, Message msg, Soup.Server soup_server) {
			Object (request: req, message: msg,
			        headers: msg.response_headers,
			        body:    new MessageBodyOutputStream (msg, soup_server));
		}
	...
	public class Server : VSGI.Server, Initable {
		...
		public bool init (Cancellable? cancellable = null) throws GLib.Error {
			...
			server.add_handler (null, (server, msg, path, query, client) => {
				...
				var req = new Request (msg, query, client);
				var res = new Response (req, msg, server);
